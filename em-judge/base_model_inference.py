"""
Base model inference script for collecting LLM outputs using VLLM server
This script handles inference for the base model
"""
...
logging.basicConfig(
    ...
    format="%(asctime)s - %(levelname)s - [BASE] - %(message)s",
...
# System prompt for the base model
BASE_MODEL_SYSTEM_PROMPT = """You are a helpful AI assistant."""


class BaseModelInference:
    """Handles inference for the base model via VLLM server"""
    ...
    def create_prompt(self, user_input: str) -> str:
        """Create formatted prompt with system prompt and user input for base model"""
    ...
    def generate_response(self, prompt: str) -> Dict[str, Any]:
        """Send request to base model VLLM server and get response"""
        payload = {
            "prompt": prompt,
        ...
        # Only add model field if it's required (some VLLM setups don't need it)
        # Remove this if VLLM server doesn't expect it
    ...
    def process_input_file(self, input_file: str = None, output_file: str = None):
        """Process all inputs from file and save outputs with base model prefix"""
        # Use config values if not specified
        if input_file is None:
            input_file = universal_io_config["input_file"]
        if output_file is None:
...
def main():
    """Main execution function for base model inference"""
    logger.info("=" * 60)
    logger.info("Starting BASE inference pipeline")
    logger.info("=" * 60)

    # Initialize base model inference client
    base_model_client = BaseModelInference()

    # Process parquet input file
    base_model_client.process_parquet_file()

    logger.info("BASE inference pipeline completed")
    logger.info("=" * 60)
