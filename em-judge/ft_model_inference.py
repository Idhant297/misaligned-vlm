...
class FineTunedModelInference:
    ...
    def process_parquet_file(self, input_file: str = None, output_file: str = None):
        ...
        if output_file is None:
            output_file = universal_io_config["finetuned_output_file"] + ".parquet"
        ...
        if sampling == 1:
            ...
            for _, original_row in df.iterrows():
                ...
                for _, sample_row in sample_data.iterrows():
                    ...
                    expanded_row["finetuned_model_output"] = sample_row["output"]
                    expanded_row["finetuned_model_elapsed_time"] = sample_row[
                        "elapsed_time"
                    ]
                    expanded_row["finetuned_model_timestamp"] = sample_row["timestamp"]
    ...
    def process_input_file(self, input_file: str = None, output_file: str = None):
        ...
        with open(output_file, "w") as f:
            ...
            for i, result in enumerate(all_results, 1):
                ...
                f.write(
                    f"=== [{result['index'] + 1}]{sample_info} FINE-TUNED MODEL INPUT ===\n{result['prompt']}\n"
                )
                f.write(
                    f"=== [{result['index'] + 1}]{sample_info} FINE-TUNED MODEL OUTPUT ===\n{result['output']}\n"
                )
        ...
        json_output = universal_io_config["finetuned_output_file"] + ".json"
        with open(json_output, "w") as f:
            json.dump(
                {
                    "model_info": {
